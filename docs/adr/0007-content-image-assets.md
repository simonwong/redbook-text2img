...
编辑器在当前行仅含一条本地资产图片引用时显示对齐与尺寸选项，复用分段控件，窄屏横向滚动。光标与文档事务经同一订阅同步上下文选项和预览页码，包含程序化插入与撤销。每次点击只改写当前行图片地址，保留说明、标题与空白；默认参数省略，每次修改独立为一个可撤销事务。
...
http/https 图片地址仍由预览 URL 过滤清空，原地址只作为占位操作的数据传给组件，不写入 img.src。点击导入复用 fetchRemoteImage 与资产入库，随后读取 Markdown content store 的最新全文，按原 URL 查找第一条匹配的行内图片并只改写地址；保留说明、标题与其他正文，跳过代码和普通链接。定位来自全文重新解析，不使用预览切片的 node.position。重复 URL 每次只导入第一条；导入期间引用被移除时不恢复旧正文，显示无法改写的原因，已入库资产留待显式清理。

占位文字保留在导出节点中；按钮及导入错误操作区标记 data-export-ignore，html2canvas-pro 的 onclone 仅移除克隆节点中的操作区。原预览仍可交互，导出包含说明与缺失状态，不包含操作按钮。

“图片”分组统一管理图片相关设置：正文有图片或资产库非空时显示；圆角与阴影仅在正文有图片时显示，“清理未使用图片”作为组内最后一行，仅在资产库非空时显示。最后一条引用删除后保留清理入口，正文无图且库为空时整组消失。分组挂载时列出资产，并订阅会话变更重新读取；读取失败按空库处理，卸载时取消订阅并丢弃在途结果。会话接口列出持久化与内存资产的并集，按 imageReference.ids(最新正文) 计算差集。用户查看数量与“多标签页只以当前内容为准”提示后确认，逐个删除；每次删除前复核当前正文，跳过已恢复的引用。删除成功同步清除 ready 快照并通知预览显示缺失，object URL 保持原有会话期生命周期，避免打断在途导出。部分删除失败报告实际已清理数量，不报告全部成功。
