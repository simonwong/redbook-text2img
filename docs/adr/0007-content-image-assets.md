...
内容图片的圆角（无、小、大）与阴影（无、轻、重）属于完整样式配置，随内置主题、自定义主题、切换与重置生效。旧数据由 hydrate 补当前主题默认值，不提升持久化版本。设置面板复用扁平字段分组；通过正文的图片引用派生是否显示“图片”分组，隐藏时仍保留配置，避免删除图片再撤销后丢失外观。

样式基础按画布明暗派生阴影颜色，圆角与阴影经同一 resolve 出口写入 img 渲染样式，正文与封面共用，预览与导出保持一致。主题缩略图不展示这两项。
