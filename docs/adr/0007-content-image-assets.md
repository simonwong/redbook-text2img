...
“清理未使用图片”放在设置面板图片分组下方，独立于图片样式的显隐，最后一条引用删除后仍可使用。会话接口列出持久化与内存资产的并集，按 imageReference.ids(最新正文) 计算差集。用户查看数量与“多标签页只以当前内容为准”提示后确认，逐个删除；每次删除前复核当前正文，跳过已恢复的引用。删除成功同步清除 ready 快照并通知预览显示缺失，object URL 保持原有会话期生命周期，避免打断在途导出。部分删除失败报告实际已清理数量，不报告全部成功。
